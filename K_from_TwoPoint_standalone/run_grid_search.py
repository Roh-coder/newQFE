#!/usr/bin/env python3
"""
run_grid_search.py — Hybrid GC3 + Z²/Z⁴ grid search
=====================================================
User-facing entry point for the K_from_TwoPoint adaptive grid search.

Uses the hybrid strategy proven in testing:
  - GC3 (3-pass Gram-Charlier) for the β_c finder
  - Normalised Z² (boundary_slices_normed) for grid evaluation (lowest CoV)
  - Normalised Z⁴ (boundary_slices_normed_quartic) re-scoring of top
    candidates at each level (steepest landscape for final discrimination)
  - Translate-or-refine grid logic (no overshoot)
  - Sum-of-squares per-direction cost aggregation
...
# ---------------------------------------------------------------------------
R1_INIT    = 1.5   # starting grid centre, r1 = k1/k3
R2_INIT    = 1.5   # starting grid centre, r2 = k2/k3
HALF_SPAN  = 0.30  # half-width of grid in r-space (e.g. 0.15 → grid ±15%)
...
# For production quality, use 50000 / 30000 / 60000.
# For high-statistics hybrid, try 100000 / 50000 / 100000.
# ---------------------------------------------------------------------------
N_TRAJ_PROD         = 50000   # trajectories for production (two-point function)
N_TRAJ_SCAN_COARSE  = 30000   # trajectories per beta point during coarse scan
N_TRAJ_SCAN_FINE    = 60000   # trajectories per beta point during fine scan
...
# ---------------------------------------------------------------------------
# Re-scoring: number of top Z² candidates per level to re-score with Z⁴.
# No additional MC runs needed (reuses existing two-point data).
# ---------------------------------------------------------------------------
TOP_N = 3
...
# ---------------------------------------------------------------------------
OUTPUT_DIR = "results/equilateral_from_1.5"
...
# 500000 gives good statistics; 1000000 for high-statistics runs.
REF_N_TRAJ = 500000

# Exact beta_c for the reference lattice.  When set (not None), the
# susceptibility scan is skipped entirely during reference generation.
# For the equilateral triangular Ising model: ln(3)/4 ≈ 0.27465307216702745.
# Leave as None to let the scan find beta_c automatically.
...
# ---------------------------------------------------------------------------
# Auto-build the binary if missing
# ---------------------------------------------------------------------------
...
for level in range(MAX_ITER):
    ...
    # --- Border detection (translate-or-refine) ---
