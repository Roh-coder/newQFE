#!/usr/bin/env python3
"""
run_grid_search_hybrid.py — Hybrid GC3 + Z²/Z⁴ grid search
============================================================
Uses GC3 (3-pass Gram-Charlier) for the β_c finder, normed Z² for the
grid search scoring, and normed Z⁴ to re-score the top candidates at
each level.  Uses the simplified grid logic (translate OR refine, never
both; span unchanged on translation).

This is an 8×8 equilateral convergence test starting at r₁ = r₂ = 1.5.
"""
...
N_GRID   = 5           # 5×5 grid per level
MAX_ITER = 6           # max grid levels
TOP_N    = 3           # re-score top N with Z⁴ per level

REF_N_TRAJ = 1000000
REF_BETA_C = None      # set to skip ref susceptibility scan

OUTPUT_DIR = "results/hybrid_gc3_z2z4_ref16_test8"
...
# ---------------------------------------------------------------------------
# Reference generation (reuse from run_grid_search_8x8.py)
# ---------------------------------------------------------------------------
_ref_tag = f"ref_{REF_Lx}x{REF_Ly}_{REF_N_TRAJ//1000}k" + (f"_t{REF_Tx}x{REF_Ty}" if REF_Tx or REF_Ty else "")
...
# ---------------------------------------------------------------------------
# Evaluate one grid point: GC3 → production → Z² cost (+ cache test_data)
# ---------------------------------------------------------------------------
def evaluate_point_gc3(r1, r2, beta_guess, level, i, j, data_dir):
    ...
    # Extract boundary slices for live plot
    slices = None
    try:
...
# ---------------------------------------------------------------------------
# Grid search with hybrid scoring
# ---------------------------------------------------------------------------
print(f"\n{'='*70}")
...
for level in range(MAX_ITER):
    ...
    grid = {}  # (i,j) -> {r1, r2, beta_c, z2, a2a_path}
...
# Find overall best across all levels
