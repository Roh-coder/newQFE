...
# ===================================================================
# Bayesian-optimisation backend (Speedup 5)
# ===================================================================

def run_bo(evaluator, x0=(1.0, 1.0), *,
           max_evals=30,
           bounds=((0.3, 1.7), (0.3, 1.7)),
           n_init=8, init_sigma=0.25,
           kappa=2.0, n_restarts=5,
           refit_every=1,
           snr_floor=0.0, indist_stop_snr=0.0,
           snr_target=0.0, snr_max_traj_factor=4,
           seed=None):
    """Surrogate-assisted optimiser (LCB on a heteroscedastic Matérn-5/2 GP).

    Strategy:
      1.  Seed the surrogate with ``n_init`` evaluations: ``x0`` plus
          ``n_init - 1`` Latin-hypercube-jittered points around ``x0``
          with std ``init_sigma`` (clipped to ``bounds``).
      2.  After every batch of ``refit_every`` evaluations, refit the GP
          and propose the next point by minimising LCB(μ − κσ) over a
          coarse grid + local polish on ``bounds``.
      3.  Stop when ``max_evals`` is hit or any of the SNR-based rules
          fire (same shutdown semantics as ``run_nelder_mead``).

    The same evaluator interface as the other backends is used; the
    GP only sees ``(r1, r2, cost, sigma_cost)``.
    """
    from surrogate import MaternGP, lcb, propose_next
    ...
    # ---- initial seed ------------------------------------------------
    init_points = [tuple(x0)]
    while len(init_points) < int(n_init):
        cand = (
            float(np.clip(x0[0] + init_sigma * rng.standard_normal(),
                          bounds[0][0], bounds[0][1])),
            float(np.clip(x0[1] + init_sigma * rng.standard_normal(),
    ...
    def _eval(x):
        ...
        if hasattr(evaluator, "current_gaussian"):
            evaluator.current_gaussian = None
        r = evaluator(float(x[0]), float(x[1]))
        history.append(r)
        # Adaptive trajectory ratchet (same rule as NM/CMA backends).
        if snr_target > 0.0 and r.snr < snr_target:
            new_n = min(_n_traj_max, int(evaluator.n_traj_prod * 1.5))
            if new_n > evaluator.n_traj_prod:
                evaluator.n_traj_prod = new_n
        return r

    status = "max_evals reached"
    ...
    try:
        ...
        gp = MaternGP(n_dim=2)
        for step in range(max_evals - len(history)):
            # Refit periodically.
            if step % max(1, int(refit_every)) == 0:
                X = np.array([[r.r1, r.r2] for r in history], dtype=float)
                y = np.array([r.cost for r in history], dtype=float)
                s = np.array([max(r.sigma_cost, 1e-6) for r in history], dtype=float)
                try:
                    gp.fit(X, y, s, n_restarts=n_restarts, seed=int(rng.integers(1 << 30)))
                except Exception as exc:
                    print(f"[bo] WARNING: GP refit failed ({exc!r}); "
                          f"falling back to random sample")
                    nxt = (float(rng.uniform(*bounds[0])),
                           float(rng.uniform(*bounds[1])))
                    _eval(nxt)
                    continue
            nxt = propose_next(lambda Z: lcb(gp, Z, kappa=kappa),
                               bounds=bounds, seed=int(rng.integers(1 << 30)))
            # Publish GP surface so the visualiser can draw it.
            if hasattr(evaluator, "current_gp_surface"):
                try:
                    _g1 = np.linspace(bounds[0][0], bounds[0][1], 30)
                    _g2 = np.linspace(bounds[1][0], bounds[1][1], 30)
                    _R1, _R2 = np.meshgrid(_g1, _g2)
                    _Z = np.column_stack([_R1.ravel(), _R2.ravel()])
                    _mu, _std = gp.predict(_Z)
                    _acq = lcb(gp, _Z, kappa=kappa)
                    evaluator.current_gp_surface = {
                        "r1": _R1, "r2": _R2,
                        "mean": _mu.reshape(_R1.shape),
                        "std":  _std.reshape(_R1.shape),
                        "acq":  _acq.reshape(_R1.shape),
                        "next": nxt,
                        "step": step,
                    }
                except Exception:
                    evaluator.current_gp_surface = None
            _eval(nxt)

            # SNR-based early stop (mirrors NM/CMA behaviour).
            if len(history) >= 5:
                best_snr = max(r.snr for r in history)
                if snr_floor > 0.0 and best_snr < snr_floor:
                    status = f"SNR floor: best={best_snr:.2f} < {snr_floor}"
                    break
                if (indist_stop_snr > 0.0 and best_snr < indist_stop_snr
                        and evaluator.n_traj_prod >= _n_traj_max):
                    status = (f"indistinguishable from reference "
                              f"(best SNR {best_snr:.2f} < {indist_stop_snr})")
                    break
