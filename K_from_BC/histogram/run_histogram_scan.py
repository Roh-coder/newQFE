...
def compile_program(workspace_root):
    ...
    source = workspace_root / "K_from_BC" / "histogram" / "tri_histogram_reweight.cc"
...
def parse_dat_files(output_dir, target_N=None):
    """Parse .dat files and return (N, beta, m, m2, action) rows.
...
def main():
    ...
    for N in sizes:
        ...
        try:
            ...
            data = parse_dat_files(".", target_N=N)
    ...
    with open(output_path, "w") as f:
        ...
        f.write("# where m = |<s>|, m2 = m^2 for each configuration\n")
        f.write("# Binder cumulant should be computed from ensemble moments:\n")
        f.write("#   U4 = 1 - <m^4> / (3 * <m^2>^2)\n")
        f.write("# Susceptibility for |m| requires separate analysis conventions.\n")
        f.write("#\n")
