#!/usr/bin/env python3
"""Compare 2D MC Ising correlator (Nt=1, K=K_2d_crit) to 2D modular Ising CFT.

4-panel figure:
  Panel 0 : MC data           — log10 connected correlator, 2×2 tiled Delaunay
  Panel 1 : Modular Ising 2D  — same colour scale; alpha fit in r∈[5%,10%]·L band
  Panel 2 : Ratio MC / theory — linear, centred on 1
  Panel 3 : Z-score tension   — (G_MC - alpha*G_th) / err_MC

The 2D Ising simulation was run at the exact triangular-lattice critical coupling
  K_c = ln(3)/4 ≈ 0.27465
on the same spatial torus (Lx=39, Ly=48, Tx=9, Ty=-9) as the 3D Nt scan.

Modular Ising CFT prediction:  arXiv:2209.15546 eq.(57)
"""
...
ALPHA_RMIN_FRAC  = 0.05   # lower end of fit band (fraction of sqrt(Ncell))
ALPHA_RMAX_FRAC  = 0.10   # upper end of fit band
K2D_CRIT         = math.log(3) / 4   # ≈ 0.27465
DATADIR          = Path(".")
OUTFILE          = DATADIR / "compare_modular_ising_2d.png"

# ── data discovery ────────────────────────────────────────────────────────────

def find_2d_dat(datadir):
    """Find the two_point_all_to_all.dat for the 2D (Nt=1, K≈0.27465) run."""
    # The binary nests inside a sub-directory named by k values; search broadly
    candidates = list(Path(datadir).glob(
        "Lx39_Ly48_Tx9_Ty-9_Nt1_k2d*/*/two_point_all_to_all.dat"))
    if candidates:
...
def fit_alpha(m, n, g_mc, g_th, eg_mc,
    ...
    """WLS alpha restricted to the r∈[5%,10%]·L band (near-cutoff)."""
    L = np.sqrt(float(LX * LY + TX * TY))
    r = np.sqrt(m * m + m * n + n * n)
    rmin, rmax = rmin_frac * L, rmax_frac * L
...
def make_tiled(m, n, z, Lx=LX, Ly=LY, Tx=TX, Ty=TY):
    ...
    e01 = verts[:, 1] - verts[:, 0]; e12 = verts[:, 2] - verts[:, 1]
    e20 = verts[:, 0] - verts[:, 2]
...
dat_path = find_2d_dat(DATADIR)
if dat_path is None:
    sys.exit("ERROR: Could not find 2D two_point_all_to_all.dat. "
             "Run the simulation first.")
print(f"Using data: {dat_path}")

m, n, g_mc, eg_mc = load_spatial(dat_path)

tau, b_sign = modular_tau()
print(f"tau = {tau.real:.6f} + {tau.imag:.6f}i  (b_sign={b_sign})")
print(f"Computing modular Ising theory for {len(m)} sites at dps={MP_DPS}...")
g_theory = compute_theory(m, n, tau, b_sign)
print("Theory computation done.")

alpha = fit_alpha(m, n, g_mc, g_theory, eg_mc)
print(f"alpha (r-band fit) = {alpha:.4e}")
g_th_scaled = alpha * g_theory

# ── derived fields ────────────────────────────────────────────────────────────
# Shared log colour limits
all_logvals = []
pos_mc = g_mc > 0
if pos_mc.any():
    all_logvals.append(np.log10(g_mc[pos_mc]))
...
ax_mc.set_title(f"2D MC data  (Nt=1, K={K2D_CRIT:.5f})", fontsize=10)
...
ax_th.set_title(f"Modular Ising CFT (α={alpha:.3e}, r∈[5%,10%]·L fit)",
                fontsize=10)
...
if ratio_finite.size > 0:
    ...
    r_lo = max(0.01, rmed - 3 * rdev); r_hi = rmed + 3 * rdev
else:
    r_lo, r_hi = 0.5, 2.0
...
ax_rat.set_title("Ratio MC / Theory", fontsize=10)
...
zs_absmax = max(float(np.percentile(np.abs(zs_finite), 98)), 0.5) if zs_finite.size > 0 else 3.0
...
ax_zs.set_title("Z-score tension", fontsize=10)
...
chi2_dof = (float(np.mean(((g_mc[fit_mask] - g_th_scaled[fit_mask])
                            / eg_mc[fit_mask]) ** 2))
            if fit_mask.any() else np.nan)

fig.suptitle(
    f"4-5-6 torus (Lx=39, Ly=48, Tx=9, Ty=-9)  —  "
    f"2D MC (K_c = ln3/4 ≈ {K2D_CRIT:.5f}) vs. 2D modular Ising CFT\n"
