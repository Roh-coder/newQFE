#!/usr/bin/env python3
"""Per-Nt comparison: 2D-critical-coupling MC vs 2D modular Ising CFT.

Runs at K = ln(3)/4 ≈ 0.27465 (stored as k0.275 in filenames).
One PNG per Nt, 4 panels each:
...
Geometry: Lx=39  Ly=48  Tx=9  Ty=-9   (4-5-6 torus, K_2d_crit)
"""
...
def find_files(datadir):
    """Return sorted (Nt, path) for k0.275 data, deduplicating by Nt."""
    result = {}
    # flat layout (how run_k2dcrit_nt_scan.sh places output)
    for p in sorted(Path(datadir).glob(
            "Lx39_Ly48_Tx9_Ty-9_Nt*_k0.275*/two_point_all_to_all.dat")):
...
def make_tiled(m, n, z, Lx=LX, Ly=LY, Tx=TX, Ty=TY):
    ...
    for ia in range(2):
        for ib in range(2):
            mi = m + ia * vm + ib * um; ni = n + ia * vn + ib * un
...
if not files:
    sys.exit("No two_point_all_to_all.dat files found in k0.275 directories.")
...
# Shared log colour limits across all Nt
all_logvals = []
...
for nt, path in files:
    ...
    fig.suptitle(
        f"4-5-6 torus (Lx=39, Ly=48, Tx=9, Ty=-9), Nt={nt}, "
        f"K=K_{{2d}}={K_LABEL}  —  MC spatial correlator vs. 2D modular Ising CFT\n"
