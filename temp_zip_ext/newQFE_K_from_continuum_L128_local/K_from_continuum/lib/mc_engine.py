...
def _auto_seed() -> int:
    return (time.time_ns() ^ (os.getpid() << 16) ^ next(_SEED_COUNTER)) & 0xFFFFFFFF


def _mc_log(message: str) -> None:
    """Emit a flushed, timestamped MC progress line."""
    stamp = time.strftime("%H:%M:%S")
    print(f"[mc {stamp}] {message}", flush=True)
...
def _is_all_to_all_name(name: str) -> bool:
    lower = name.lower()
    return lower.endswith(".dat") and ("all_to_all" in lower)


def _collect_all_to_all_files(root_dir: str) -> list[str]:
    files: list[str] = []
    if not os.path.isdir(root_dir):
        return files
    for root, _, names in os.walk(root_dir):
        for name in names:
            if _is_all_to_all_name(name):
                files.append(os.path.join(root, name))
    return files


def _parse_output_subdir_from_stream(stream_text: str) -> str | None:
    marker = "opening file:"
    for line in stream_text.splitlines():
        ...
        if idx < 0:
            continue
        full_path = line[idx + len(marker):].strip().strip('"').strip("'")
        if not full_path:
            continue
        subdir = os.path.dirname(os.path.normpath(full_path))
        if subdir:
            return subdir
    return None


def _extract_data_dirs_from_stream(stream_text: str) -> list[str]:
    marker = "data_dir:"
    out: list[str] = []
    for line in stream_text.splitlines():
        lower = line.lower()
        ...
        if idx < 0:
            continue
        data_dir = line[idx + len(marker):].strip().strip('"').strip("'")
        if data_dir:
            out.append(os.path.normpath(data_dir))
    return out


def _recover_output_subdir_from_filesystem(
    data_dir: str,
    ...
    for root in roots:
        ...
        candidates.extend(_collect_all_to_all_files(norm_root))

    candidates = [os.path.normpath(path) for path in candidates]
    candidates = sorted(set(candidates))
    if not candidates:
        return None

    # Prefer files that appeared during this run; otherwise take newest known file.
    new_candidates = [path for path in candidates if path not in before_files]
    pool = new_candidates if new_candidates else candidates
    newest = max(pool, key=lambda p: os.path.getmtime(p) if os.path.exists(p) else -1.0)
...
def run_simulator(exe, Lx, Ly, Tx, Ty, k1, k2, k3, beta,
                  ...
                  wall_time=0.0, summary_only=False):
    """Run the C++ simulator and return (stdout_text, subdir_path)."""
    seed = _auto_seed() if seed == 0 else seed
    os.makedirs(data_dir, exist_ok=True)
    before_files = set(_collect_all_to_all_files(data_dir))
    cmd = [
        exe,
    ...
    _mc_log(
        "run_simulator start "
        f"L=({Lx},{Ly}) T=({Tx},{Ty}) "
        f"k=({k1:.6g},{k2:.6g},{k3:.6g}) beta={beta:.8f} "
        f"n_traj={n_traj} n_skip={n_skip} n_therm={n_therm}"
    )
    ...
    subdir = _parse_output_subdir_from_stream(result.stdout)
    if subdir is None:
        subdir = _parse_output_subdir_from_stream(result.stderr)

    extra_roots = (
        _extract_data_dirs_from_stream(result.stdout)
        + _extract_data_dirs_from_stream(result.stderr)
    )

    if subdir is None:
        subdir = _recover_output_subdir_from_filesystem(
            data_dir,
            before_files,
            extra_roots=extra_roots,
        )

    if subdir is None and result.stderr.strip():
        _mc_log(
            "run_simulator warning "
            f"(stderr): {result.stderr.strip().splitlines()[-1]}"
        )

    _mc_log(
        "run_simulator done "
        f"beta={beta:.8f} n_traj={n_traj} wall={time.time() - start_t:.1f}s "
        f"output={subdir if subdir is not None else '<unknown>'}"
    )
...
def find_beta_c(exe, Lx, Ly, Tx, Ty, k1, k2, k3,
    ...
    _mc_log(
        "find_beta_c start "
        f"L=({Lx},{Ly}) T=({Tx},{Ty}) "
        f"k=({k1:.6g},{k2:.6g},{k3:.6g}) "
        f"beta=[{beta_lo:.6f},{beta_hi:.6f}]"
    )
    ...
    def _record_beta_result(key, b, chi, chi_err, n_traj):
        scan_betas.append(b)
        ...
        _mc_log(
            "beta scan "
            f"pass={_current_pass[0]} "
            f"pt={_pass_counter[0]}/{_pass_counter[1]} "
            f"beta={b:.8f} chi={chi:.6g} +/- {chi_err:.3g} "
            f"traj_done={_traj_done[0]}"
        )
    ...
    def _run_beta_once(b, n_traj):
        key = round(b, 10)
        stdout, _ = run_simulator(exe, Lx, Ly, Tx, Ty, k1, k2, k3, b,
                      n_traj=n_traj, data_dir=data_dir,
                      summary_only=True, seed=_auto_seed())
        chi, chi_err = parse_stdout_value_with_err(stdout, "m_susc:")
        return key, b, chi, chi_err

    def _run_beta(b, n_traj):
        key = round(b, 10)
        if key in ran:
            return ran[key]
        key, beta_val, chi, chi_err = _run_beta_once(b, n_traj)
        return _record_beta_result(key, beta_val, chi, chi_err, n_traj)

    def _run_beta_batch(betas, n_traj):
        pending = []
        for b in betas:
            beta_val = float(b)
            key = round(beta_val, 10)
            if key in ran:
                continue
            pending.append(beta_val)
        if not pending:
            return
        if int(n_workers) <= 1 or len(pending) == 1:
            for beta_val in pending:
                _run_beta(beta_val, n_traj)
            return

        with ThreadPoolExecutor(max_workers=min(int(n_workers), len(pending))) as pool:
            futures = [pool.submit(_run_beta_once, beta_val, n_traj) for beta_val in pending]
            for future in as_completed(futures):
                key, beta_val, chi, chi_err = future.result()
                _record_beta_result(key, beta_val, chi, chi_err, n_traj)
    ...
    for shift_iter in range(max_shifts + 1):
        ...
        _run_beta_batch(np.linspace(beta_lo, beta_hi, n_coarse), n_traj_coarse)
    ...
    _run_beta_batch(fine_betas, n_traj_fine)
    ...
    _run_beta_batch(fine2_betas, n_traj_fine)
    ...
    _run_beta_batch(fine3_betas, n_traj_fine)
    ...
    _mc_log(
        "find_beta_c done "
        f"beta_c={beta_c_final:.8f} sigma={beta_c_sigma:.3g} "
        f"chi_peak={chi_peak:.6g} scan_pts={len(scan_betas)} "
        f"traj_total={_traj_done[0]} wall={time.time() - _t_start:.1f}s"
    )
...
def find_beta_c_reweight(exe, Lx, Ly, Tx, Ty, k1, k2, k3,
    ...
    _mc_log(
        "find_beta_c_reweight start "
        f"L=({Lx},{Ly}) T=({Tx},{Ty}) "
        f"k=({k1:.6g},{k2:.6g},{k3:.6g}) "
        f"beta=[{beta_lo:.6f},{beta_hi:.6f}] "
        f"n_traj_parent={n_traj_parent} n_grid={n_grid}"
    )
    ...
    for recenter_iter in range(max_recenters + 1):
        ...
        _mc_log(
            "reweight parent run "
            f"iter={recenter_iter + 1}/{max_recenters + 1} "
            f"beta_parent={beta_parent:.8f} n_traj={n_traj_parent}"
        )
        cmd = [
            exe,
            "--L_x", str(Lx), "--L_y", str(Ly),
        ...
        _mc_log(f"reweight parent traces ready: {traces_path}")
        ...
        _mc_log(
            "reweight scan "
            f"iter={recenter_iter + 1}/{max_recenters + 1} "
            f"valid={int(valid.sum())}/{n_grid} beta_est={beta_c_est:.8f}"
        )
    ...
    _mc_log(
        "find_beta_c_reweight done "
        f"beta_c={beta_c_final:.8f} sigma={beta_c_sigma:.3g} "
        f"chi_peak={chi_peak:.6g} wall={time.time() - t_start:.1f}s"
    )
...
def _spawn_parent_traces(exe, Lx, Ly, Tx, Ty, k1, k2, k3, beta_parent,
    ...
    _mc_log(
        "spawn_parent start "
        f"beta={beta_parent:.8f} n_traj={n_traj} "
        f"L=({Lx},{Ly}) T=({Tx},{Ty})"
    )
    cmd = [
        exe,
    ...
    _mc_log(
        "spawn_parent done "
        f"beta={beta_parent:.8f} n_traj={n_traj} wall={time.time() - t0:.1f}s "
...
def find_beta_c_multidonor(exe, Lx, Ly, Tx, Ty, k1, k2, k3,
    ...
    _mc_log(
        "find_beta_c_multidonor start "
        f"L=({Lx},{Ly}) T=({Tx},{Ty}) "
        f"k=({k1:.6g},{k2:.6g},{k3:.6g}) "
        f"beta=[{beta_lo:.6f},{beta_hi:.6f}] n_traj_parent={n_traj_parent}"
    )
    ...
    _mc_log(
        "find_beta_c_multidonor done "
        f"beta_c={beta_c_final:.8f} sigma={beta_c_sigma:.3g} "
        f"chi_peak={chi_peak:.6g} donors={len(rws)}"
    )
...
def find_beta_c_multidonor_2pass(exe, Lx, Ly, Tx, Ty, k1, k2, k3,
    ...
    _mc_log(
        "find_beta_c_multidonor_2pass start "
        f"L=({Lx},{Ly}) T=({Tx},{Ty}) "
        f"k=({k1:.6g},{k2:.6g},{k3:.6g}) "
        f"beta=[{beta_lo:.6f},{beta_hi:.6f}] n_traj_parent={n_traj_parent}"
    )
    ...
    _mc_log(
        "find_beta_c_multidonor_2pass done "
        f"beta_c={beta_c_final:.8f} sigma={beta_c_sigma:.3g} "
        f"chi_peak={chi_peak:.6g} p1_donors={len(rws_p1)} p2_donors={len(rws_p2)}"
    )
