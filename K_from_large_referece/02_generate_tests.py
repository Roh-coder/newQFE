...
def _point_worker(job: dict[str, Any]) -> dict[str, Any]:
    ...
    for size_job in job["size_jobs"]:
        ...
        if (
            ...
            and _can_reuse_continuum_payload(metadata, continuum_beta)
        ):
            if os.path.exists(legacy_pickle_path):
                os.remove(legacy_pickle_path)
            job_results.append(
...
def main() -> None:
    ...
    if beta_strategy_mode not in {"per_size_beta", "free_power_continuum"}:
        raise ValueError(
            "beta_strategy.mode must be one of {'per_size_beta', 'free_power_continuum'}"
        )
    ...
    if beta_strategy_mode == "free_power_continuum" and len(sizes) < 3:
        raise ValueError("beta_strategy.mode=free_power_continuum requires at least 3 test sizes")
    ...
    if beta_strategy_mode == "per_size_beta":
        ...
        log(
            f"Running {len(point_jobs)} coupling-point jobs with workers={workers} "
            f"using beta_strategy.mode={beta_strategy_mode}"
        )
        if workers == 1:
            ...
            with ProcessPoolExecutor(max_workers=workers) as pool:
                futures = [pool.submit(_point_worker, point_job) for point_job in point_jobs]
                for future in as_completed(futures):
                    ...
                    point_status = "ok" if all(job["status"] in {"ok", "skip"} for job in point_result["jobs"]) else "err"
                    log(
                        f"[{done}/{len(point_jobs)}] r1={point_result['r1']:.6f} "
                        f"r2={point_result['r2']:.6f} status={point_status}"
                    )
