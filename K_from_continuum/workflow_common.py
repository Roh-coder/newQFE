...
def _beta_continuum_taylor2_model(
    L: np.ndarray, beta_c: float, linear: float, quadratic: float
) -> np.ndarray:
    invL = 1.0 / np.asarray(L, dtype=float)
    return beta_c + linear * invL + quadratic * invL * invL
...
def fit_beta_continuum_taylor2(
    L_values: list[int] | np.ndarray,
    beta_values: list[float] | np.ndarray,
    sigma_values: list[float] | np.ndarray | None = None,
    *,
    ...
    if len(L) < 3:
        raise ValueError("taylor2 continuum beta fit requires at least 3 sizes")

    invL = 1.0 / L
    X = np.column_stack([np.ones_like(invL), invL, invL * invL])
    if weighted:
        good = sigma > 0.0
        if int(np.count_nonzero(good)) < 3:
            raise ValueError(
                "weighted taylor2 continuum beta fit requires at least 3 positive sigma values"
            )
        L = L[good]
        beta = beta[good]
        sigma = sigma[good]
    ...
    beta_fit = _beta_continuum_taylor2_model(L, *coeffs)
    residual = beta - beta_fit
    variance = float(cov[0, 0]) if np.isfinite(cov[0, 0]) else np.nan
    beta_c_sigma = float(np.sqrt(variance)) if np.isfinite(variance) and variance >= 0.0 else None

    return {
        "method": "taylor2_continuum",
        "ansatz": "beta_c(L) = beta_c(infinity) + a1/L + a2/L^2",
        "weighted": bool(weighted),
        "parameters": {
            "beta_c": float(coeffs[0]),
            "linear": float(coeffs[1]),
            "quadratic": float(coeffs[2]),
        },
        "L_values": [int(x) for x in L],
        "beta_values": [float(x) for x in beta],
        "sigma_values": [float(x) for x in sigma],
        "beta_fit_values": [float(x) for x in beta_fit],
        "residual_values": [float(x) for x in residual],
        "beta_c_continuum": float(coeffs[0]),
        "beta_c_continuum_sigma": beta_c_sigma,
        "rms_residual": float(np.sqrt(np.mean(residual * residual))),
        "n_points": int(len(L)),
        "lmin": int(np.min(L)),
...
def _observable_power_model(L: np.ndarray, A: float, B: float, C: float) -> np.ndarray:
    return A + B * np.power(1.0 / np.asarray(L, dtype=float), C)


def _observable_taylor2_model(L: np.ndarray, A: float, B: float, C: float) -> np.ndarray:
    invL = 1.0 / np.asarray(L, dtype=float)
    return A + B * invL + C * invL * invL


def evaluate_observable_fit(invL_values: np.ndarray, A: float, B: float, C: float, fit_mode: str) -> np.ndarray:
    invL = np.asarray(invL_values, dtype=float)
    if str(fit_mode).startswith("taylor2"):
        return A + B * invL + C * invL * invL
    return A + B * np.power(np.clip(invL, 0.0, None), C)


def _fit_observable_fixed_c(
    L: np.ndarray,
    y: np.ndarray,
    sigma: np.ndarray,
    C_fixed: float,
...
def _fit_observable_taylor2(
    L: np.ndarray,
    y: np.ndarray,
    sigma: np.ndarray,
) -> tuple[float, float, float, float, str]:
    invL = 1.0 / np.asarray(L, dtype=float)
    sigma_safe = _robust_sigma(sigma)
    w_sqrt = 1.0 / sigma_safe
...
def _robust_sigma(sigma: np.ndarray) -> np.ndarray:
    ...
    return np.where(good, sigma, med)


def _weighted_chi2(y: np.ndarray, y_fit: np.ndarray, sigma: np.ndarray) -> float:
    sigma_safe = _robust_sigma(sigma)
    resid = (np.asarray(y, dtype=float) - np.asarray(y_fit, dtype=float)) / sigma_safe
    return float(np.sum(resid * resid))


def _aicc_from_chi2(chi2: float, n: int, k: int) -> float:
    if n <= k + 1:
        return np.inf
    return float(chi2 + 2.0 * k + (2.0 * k * (k + 1)) / float(n - k - 1))


def fit_observable_continuum_power(
    L_values: np.ndarray,
    y_values: np.ndarray,
    ...
    *,
    ...
    aicc_min_improvement: float = 2.0,
    A_band_factor: float = 1.0,
    ...
    if n == 1:
        sigma_0 = float(abs(sigma[0])) if np.isfinite(sigma[0]) else np.nan
        return float(y[0]), sigma_0, 0.0, 1.0, 1, "single_point"

    fit_method = str(fit_method).strip().lower()
    if fit_method not in {"power", "taylor2"}:
        raise ValueError(f"unknown observable continuum fit method: {fit_method}")

    sigma_fit = _robust_sigma(sigma)
    A_fc, sA_fc, B_fc, C_fc, _ = _fit_observable_fixed_c(L, y, sigma_fit, 1.0)
    y_fit_fc = _observable_power_model(L, A_fc, B_fc, C_fc)
    ...
    if fit_method == "taylor2":
        try:
            A, sigma_A, B, C, fit_mode = _fit_observable_taylor2(L, y, sigma_fit)
        except Exception:
            return A_fc, sA_fc, B_fc, C_fc, n, "fallback_fixed_C_taylor2_err"
        if not all(np.isfinite(v) for v in (A, sigma_A, B, C)):
            return A_fc, sA_fc, B_fc, C_fc, n, "fallback_fixed_C_taylor2_err"
        return A, sigma_A, B, C, n, fit_mode
    ...
    y_fit_free = _observable_power_model(L, A, B, C)
    chi2_free = _weighted_chi2(y, y_fit_free, sigma_fit)
    aicc_free = _aicc_from_chi2(chi2_free, n, 3)
    sigma_floor = float(np.median(sigma_fit)) if sigma_fit.size else 0.0
    y_min = float(np.min(y))
    y_max = float(np.max(y))
    y_span = max(y_max - y_min, sigma_floor, 1e-12)
    ...
    A_out_of_band = (
        (A < (y_min - A_band_factor * y_span - 3.0 * sigma_floor)) or
        (A > (y_max + A_band_factor * y_span + 3.0 * sigma_floor))
    )
    weak_aicc_gain = not np.isfinite(aicc_free) or (aicc_free > (aicc_fc - aicc_min_improvement))

    if pinned_low or pinned_high or sigma_A_blown or A_extreme or A_out_of_band or weak_aicc_gain:
        reason = (
            "Cmin" if pinned_low else
            "Cmax" if pinned_high else
            "sigA" if sigma_A_blown else
            "Aext" if A_extreme else
            "Aband" if A_out_of_band else
            "aicc"
        )
