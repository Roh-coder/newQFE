...
- 2026-05-18 Windows/Spyder L128 repro for this bundle used `configs/local_L128_reference.json` with isotropic couplings `k1=k2=k3=1`, sizes `[16, 32, 48, 64, 80, 96, 112, 128]`, `beta_finder` trajectories `(200 coarse, 400 fine)`, and production `mc.n_traj=2000`.
- Step 1 can look stuck even after the simulator has finished. `run_from_spyder.py` prints the last parsed status snapshot whenever the child process is quiet, so repeated `phase=production MC` lines may be stale status rather than evidence that MC is still running.
- The quiet tail after the final `run_simulator done` line is likely Python post-processing in `01_generate_reference.py`, not Monte Carlo. Step 1 reloads each `two_point_all_to_all.dat` and repeatedly calls `sample_directional_channels(...)`, which rebuilds tiled `LinearNDInterpolator` objects during the reference channel/FSS fit pass.
- On 2026-05-18 the bundled `02_generate_tests.py` had gone stale and still expected `reference_family`/`reference_couplings`/`analysis`, while `configs/local_L128_tests.json` and `run_from_spyder.py` were already using the newer Step 2 `test_family`/`couplings` workflow. The bundled Step 2 driver has been resynced to the current repo version.
- Practical resume rules: if the last visible simulator line is `run_simulator done` for the smallest reference lattice, wait for the quiet analysis tail before killing the job; if Step 2 fails immediately on missing top-level sections, verify that `02_generate_tests.py` still matches the test-family/couplings schema before rerunning.