...
TEST_RUN_START_RE = re.compile(
    r"Running (?P<total>\d+) (?P<kind>test|coupling-point) jobs with workers=(?P<workers>\d+)"
    r"(?: using beta_strategy\.mode=(?P<mode>\S+))?"
)
TEST_DISPATCH_RE = re.compile(
    r"\[dispatch\] queued (?P<queued>\d+)/(?P<total>\d+) (?P<kind>test|coupling-point) jobs"
)
TEST_JOB_START_RE = re.compile(
    r"\[start (?P<index>\d+)/(?P<total>\d+)\] "
    r"(?:(?:L=(?P<L>\d+) )?)r1=(?P<r1>\S+) r2=(?P<r2>\S+)"
)
TEST_JOB_DONE_RE = re.compile(
    r"\[(?P<done>\d+)/(?P<total>\d+)\] "
    r"(?:(?:L=(?P<L>\d+) )?)r1=(?P<r1>\S+) r2=(?P<r2>\S+) status=(?P<status>\w+)"
)
...
def _format_status_snapshot(status: dict[str, object]) -> str:
    ...
    test_progress = status.get("test_progress")
    if test_progress:
        parts.append(f"test={test_progress}")
...
def _update_status_from_line(status: dict[str, object], line: str, label: str) -> str | None:
    ...
    if label == "02_generate_tests.py":
        match = TEST_RUN_START_RE.search(stripped)
        if match:
            status["step_name"] = label
            status["mc_phase"] = "test job setup"
            mode = match.group("mode")
            mode_suffix = f", mode={mode}" if mode else ""
            status["test_progress"] = (
                f"0/{match.group('total')} completed, workers={match.group('workers')}{mode_suffix}"
            )
            return _format_status_snapshot(status)

        match = TEST_DISPATCH_RE.search(stripped)
        if match:
            status["step_name"] = label
            status["mc_phase"] = "test jobs queued"
            status["test_progress"] = (
                f"queued {match.group('queued')}/{match.group('total')}, completed 0/{match.group('total')}"
            )
            return _format_status_snapshot(status)

        match = TEST_JOB_START_RE.search(stripped)
        if match:
            status["step_name"] = label
            status["mc_phase"] = "running test job"
            target = f"r1={match.group('r1')}, r2={match.group('r2')}"
            if match.group("L"):
                target = f"L={match.group('L')} " + target
            status["test_progress"] = f"starting {match.group('index')}/{match.group('total')} ({target})"
            return _format_status_snapshot(status)

        match = TEST_JOB_DONE_RE.search(stripped)
        if match:
            status["step_name"] = label
            status["mc_phase"] = "running test jobs"
            target = f"r1={match.group('r1')}, r2={match.group('r2')}"
            if match.group("L"):
                target = f"L={match.group('L')} " + target
            status["test_progress"] = (
                f"completed {match.group('done')}/{match.group('total')}; last {target} status={match.group('status')}"
            )
            return _format_status_snapshot(status)

        if stripped.startswith("Continuum beta extrapolation summary written:"):
            status["step_name"] = label
            status["mc_phase"] = "writing beta extrapolation summary"
            return _format_status_snapshot(status)

        if stripped.startswith("Test manifest written:"):
            status["step_name"] = label
            status["mc_phase"] = "writing test manifest"
            return _format_status_snapshot(status)

    analysis_marker = "[analysis]"
    if analysis_marker in stripped:
        status["step_name"] = label
        status["mc_phase"] = stripped.split(analysis_marker, 1)[1].strip() or "analysis"
