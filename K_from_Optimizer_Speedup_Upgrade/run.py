...
CONFIG = {
    ...
    # Speedup 4 — Ferrenberg–Swendsen β reweighting.  When True, the
    # per-eval β_c is found by ONE long parent MC at the bracket
    # midpoint plus reweighted χ(β) on a dense grid (replaces the
    # 3-pass scan).  See README "Speedup 4" + "Validation test panel".
    "reweight":               False,
    "reweight_n_traj":        40000,   # parent ensemble size
    "reweight_n_grid":        201,     # dense β grid resolution
    "reweight_n_eff_floor":   0.10,    # reject β where N_eff/N < this
    "reweight_max_recenters": 3,       # respawn parent if peak drifts out

    # Speedup 5 — Bayesian-optimisation surrogate ("bo" optimizer backend).
    # Used only when CONFIG["optimizer"] == "bo" — see run_bo() in
    # optimizer.py and surrogate.py.  The defaults below match the
    # Validation-panel BO sub-panel (5 starts × 5 geoms).
    "bo_bounds":              ((0.3, 1.7), (0.3, 1.7)),
    "bo_n_init":              8,
    "bo_init_sigma":          0.25,
    "bo_kappa":               2.0,
    "bo_n_restarts":          5,
...
def main():
    ...
    # ----------------------------------------------------------------
    # Validation-panel hooks (tests/validation/run_panel.py).  These
    # let an external driver override the in-file CONFIG without
    # editing this file, so panel cells run as fully isolated processes.
    #   KOPT_PNP_CFG_OVERRIDE = path to a JSON dict whose keys are
    #                           merged on top of CONFIG (tuples may be
    #                           encoded as JSON arrays).
    #   KOPT_PNP_OUTPUT_DIR   = absolute output directory; if set,
    #                           overrides results_dir + run_name.
    # ----------------------------------------------------------------
    _cfg_override = os.environ.get("KOPT_PNP_CFG_OVERRIDE")
    if _cfg_override and os.path.exists(_cfg_override):
        with open(_cfg_override) as _f:
            _patch = json.load(_f)
        # Re-tuple the keys CONFIG stores as tuples.
        for _k in ("ref_beta_seed", "beta_seed", "x0", "bo_bounds"):
            if _k in _patch and isinstance(_patch[_k], list):
                if _k == "bo_bounds":
                    _patch[_k] = tuple(tuple(p) for p in _patch[_k])
                else:
                    _patch[_k] = tuple(_patch[_k])
        cfg = {**CONFIG, **_patch}
        print(f"[run] CONFIG override loaded from {_cfg_override} "
              f"({len(_patch)} keys)")
    _out_override = os.environ.get("KOPT_PNP_OUTPUT_DIR")
    if _out_override:
        cfg["results_dir"] = os.path.dirname(os.path.abspath(_out_override))
        cfg["run_name"]    = os.path.basename(os.path.abspath(_out_override))
    ...
    ev = Evaluator(
        ...
        reweight=cfg.get("reweight", False),
        reweight_n_traj=cfg.get("reweight_n_traj", 40000),
        reweight_n_grid=cfg.get("reweight_n_grid", 201),
        reweight_n_eff_floor=cfg.get("reweight_n_eff_floor", 0.10),
        reweight_max_recenters=cfg.get("reweight_max_recenters", 3),
    ...
    if optimizer_name == "nelder_mead":
        ...
        def _run():
            return run_bo(
                ...
                x0=tuple(cfg["x0"]),
                max_evals=cfg["max_evals"],
                bounds=tuple(tuple(b) for b in cfg.get("bo_bounds", ((0.3, 1.7), (0.3, 1.7)))),
                n_init=cfg.get("bo_n_init", 8),
                init_sigma=cfg.get("bo_init_sigma", 0.25),
                kappa=cfg.get("bo_kappa", 2.0),
                n_restarts=cfg.get("bo_n_restarts", 5),
                refit_every=cfg.get("bo_refit_every", 1),
                snr_floor=cfg["snr_floor"],
                indist_stop_snr=cfg["indist_stop_snr"],
                snr_target=cfg["snr_target"],
                snr_max_traj_factor=cfg["snr_max_traj_factor"],
                seed=cfg.get("bo_seed"),
            )
    else:
        raise ValueError(
            f"Unknown optimizer {optimizer_name!r}; "
            "expected 'nelder_mead', 'cmaes', or 'bo'."
        )
